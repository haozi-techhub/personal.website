...
> 本文件定义**基础规范**（snap、入场、hover、性能）。
> 高阶招牌交互（hero 字效、磁吸光标、滚动叙事等）见 [`08-signature-interactions.md`](./08-signature-interactions.md)。

## 0. 动效哲学

- **目的优先**：每个动效都要回答「它解释了什么 / 引导到哪里」，不做装饰性消耗
- **快进慢出**：进入快（150–240ms），离开稍慢（240–360ms）
- **空间感**：用 `transform: translateZ()` 与 `perspective` 营造层次，避免 2D 平贴
- **节奏**：一屏内最多 1 个「主角动效」+ 2-3 个「配角」，其余克制
- **反馈即时**：所有可点击元素 < 100ms 必须有视觉响应
